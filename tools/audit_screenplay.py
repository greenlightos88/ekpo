...
def main() -> int:
    ...
    if len(headings) == 26:
        s2, s6, s9, s10 = sequence(2), sequence(6), sequence(9), sequence(10)
        ...
        checks.extend([
            ("Emma is first named naturally by Bode at Solomon's funeral", "BODE\nEmma. My sister." in s2 and "Emma" not in sequence(1)),
            ("Emma's perceptual notes teach indirect observation", "Emma:" in s10 and "Look again from the water." in s10),
            ...
            ("brass discovery uses distinct floor, wall and worktable staging", all(term in s18 for term in ["stone floor", "wall map", "curved worktable"])),
            ("Emma's independent observations visibly correct an earlier reading", all(term in s18 for term in ["Read the river first", "first arrow has been crossed out", "E."])),
            ...
            ("two-rhythm discovery uses distinct veranda-step reflection staging", all(term in s19 for term in ["lower veranda step", "shallow washbasin", "one step above"])),
            ("Kai's initial two-rhythm deduction is fallible and corrected", all(term in s19 for term in ["I matched the shape to the wrong sound", "Play it from before", "Emma separated them"])),
            ...
            ("Emma's final-message discovery uses distinct floor-mat staging", all(term in s21 for term in ["plain mat", "facedown on the sideboard", "angles the work lamp across the floor"])),
            ...
            ("climax establishes water direction before red-line load and Malik", s24.index("In the water cup") < s24.index("The WEST line becomes a red bar") < s24.index("Malik steps into the working circle")),
    ...
    prohibited = [
        ...
        ("Emma never appears as a speaking ghost or living flashback", r"^EMMA(?:\s*\([^\n]*\))?$"),
    ...
    production_plan = args.source.parent / "docs" / "PRESSURE_CHAMBER_PRODUCTION_AND_SAFETY_PLAN.md"
    plan_text = production_plan.read_text(encoding="utf-8") if production_plan.exists() else ""
    checks.extend([
        ("practical chamber production plan exists with fixed-guide blocking", all(term in plan_text for term in ["Practical set geography", "fixed guides", "Shot-planning minimum"])),
        ("production plan forbids real performer restraint and breathing restriction", all(term in plan_text for term in ["Never use an actor's actual neck", "never restrict actual breathing", "quick-release"])),
        ("production plan requires qualified stunt, sound and accessibility review", all(term in plan_text for term in ["qualified stunt", "sound/safety personnel", "accessibility accommodations"])),
    ])
